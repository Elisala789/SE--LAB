...
const Verified = () => {
      ...
      const filteredRequests = await Promise.all(
        response.data.map(async (req) => {
          try {
            ...
            const { supervisorVerified, forwardedToLabAdmin } = statusResponse.data;

            // Include only supervisor-verified and not yet forwarded requests
            if (supervisorVerified && !forwardedToLabAdmin) {
              return req;
            }
            return null; // Exclude other requests
          } catch (error) {
            console.error("Error checking request status:", error);
            return null; // If error occurs, exclude the request
          }
        })
      );
  ...
  return (
    <SupervisorLayout>
      <div className="p-6">
        <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">Verified Requests</h2>
        {requests.length === 0 ? (
          <p className="text-gray-600 text-center">No verified requests.</p>
        ) : (
