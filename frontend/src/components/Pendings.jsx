...
const Pendings = () => {
      ...
      const filteredRequests = await Promise.all(
        response.data.map(async (req) => {
          try {
            ...
            if (statusResponse.data.supervisorVerified) {
              setVerifiedRequests((prev) => ({ ...prev, [req.id]: true }));
              return null; // Exclude verified requests
            }
            return req;
          } catch (error) {
            console.error("Error checking request status:", error);
            return req; // If error, still include request
          }
        })
      );
  ...
  const updateStatus = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:8081/api/auth/check_status/${selectedRequest.id}`,
        null,
        {
          params: {
